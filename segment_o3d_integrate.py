...
class PointCloudSLAM:
    ...
    def integrate(self):
        ...
        #     self.global_pcd += pcd  # 누적 포인트 클라우드 업데이트
        if not pcd.is_empty():
            if self.global_pcd.is_empty():
                self.global_pcd = pcd
            else:
                # Estimate extrinsic transformation
                transformation = self.estimate_extrinsic(self.global_pcd, pcd)
    ...
    def estimate_extrinsic(self, source_cloud, target_cloud):
        ...
        reg_p2p = o3d.pipelines.registration.registration_icp(
            source_cloud, target_cloud, threshold, extrinsic,
